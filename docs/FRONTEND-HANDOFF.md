...
| `City.tsx` | The six districts | **Pinned** on desktop with scroll-scrubbed clip wipes and snapping (`inertia: false`); rows switch plates when not pinned. On lg the left column is `features/districts/RouteMap`: the section's timeline also tweens a `progress` motion value (0–5), and the map's pointer, travelled road, hue and dot-matrix light all derive from it via `routeAt()` (`geometry.ts`, unit-tested), so the pointer glides along the real route in step with the wipes. Its places are buttons (`select(i)`); **Full map** / the phones' "On the city map" button open `CityMapDialog` pointing at a district. |
...
| `components/gl/ShaderCanvas.tsx` | One tiny WebGL1 runtime, no library | Full-screen triangle. It renders only while on screen and while the tab is visible, caps DPR, draws one still frame under reduced motion, and without WebGL, on a compile/link failure or on context loss it removes or hides the canvas so the parent's CSS background shows (a lost context left on screen paints as a flat grey sheet). **Each mount creates its own `<canvas>`** and frees its GL context on unmount: reusing one element hands a remount (React Strict Mode in dev, Fast Refresh) the context it just lost. |
...
|---|---|---|
| `CallRoom.tsx` | The call UI | Reads **only** `useCallStore` and `useLiveCall`. Idle → permission → connecting → ringing → live → ending/scoring. In live mode it shows the `TalkBar` (speak or type); in simulation, scripted replies (keys 1–3). Keyboard: **M** mutes. `?auto=1` auto-answers once (read from `window.location`, not `useSearchParams`, because the page is statically generated). **Layout contract:** exactly `h-dvh`, and the transcript scrolls inside, so the reply box and End call stay visible on phones. **Speaker mode** (`speakerMode` in the call store, remembered in localStorage, loaded after mount): the Gemini provider drops mic chunks while `PcmPlayer.recentlyAudible(450)`, so the model never hears itself off the speakers; barge-in is voice-off, typing still interrupts. |
...
| `score-call.ts` | "Always get a score" | AI judge via `/api/score` (10.5 s budget; the server races its judge chain within 8.5 s with `generateJson({ hedgeMs, budgetMs })`); on any failure it uses the rules judge, which also speaks "conversation/contact" for chats (`scenarioId: "messages"`). |
| `JudgingProgress.tsx` | The wait, made legible | Time-paced stages (Reading the transcript → Identifying tactics → Building your profile) and a "taking longer" line after 7 s. Used by the call room and Messages. |
| `mock-judge.ts` | Rules judge | `judgeCall`, `PASS_THRESHOLD` (65), `fmt` (mm:ss). |
| `compose.ts` | Judge JSON → `CallScore` | The score is **computed** from the judge's `breakdown` line items (`JUDGE_BASE` 50 + Σ points), never taken from the model, then lifted by `safetyFloor()` (mock-judge.ts: 75 for blocking/exposing a scam or verifying a genuine contact with nothing revealed). Also clamps times, de-duplicates, applies the pass mark, trims labels to whole words. `ScoreReport` renders the result as **Why N?**; the rules judge produces the same shape. |
...
| `FreestylePill.tsx` | "You're still awake" | A fixed status chip (lives, progress, level). Hidden in `/play` (the call room's header carries a `FreestyleChip`) and on `/freestyle`. On phones it shrinks to lives and progress, and sets `html[data-fs-pill]` so `main` keeps 4.5rem of room for it (`globals.css`) — it must never cover a call to action. |
...
| `messages/ChatPlayer.tsx` | Social engineering by text | Phone-style chat. Each turn calls `/api/chat` (reply plus the analyst's read). The contact can end the chat itself. "Block & report" / "End conversation" → `scoreCall` → results (`channel: "sms"`). Typing dots show from the first paint. Standalone, it waits up to 2.5 s for the AI's plan, then opens with a built-in plan from `content/fallback-chats.ts` (replies are still live AI). Only a server with no AI key shows the "unavailable" state. |
...
- **`components/ui/TacticChip.tsx`:** the red-flag chip in three states (not seen yet → suspected, from the tactics the other side has used → called out). Call room and Messages.
- **District vs channel:** calls are placed by district up front (`District 02 · The Delivery · Channel · Call`). Email, web and chat pages show only `Channel · …` until the verdict, because the district names the tactic; `placeLabel()` then adds it.
- **`profile/defense.ts`** (pure, unit-tested): `defenseProfile({ weak, strong, falseAlarms, trusted, session })` → archetype (each tactic gets **one** verdict from its caught-minus-missed balance, and `session`, the report on screen, outranks history, so the page can't say "strong against urgency" beside "you missed urgency"), strong/weak tactic, next-threat district (each district has a `lever`: Authority, Urgency, Fear, Greed, Obedience, Trust). `lessonFrom(...)` → the one-line "The city noticed" evidence after an encounter. **Keep it honest:** it must only claim what `recordTactics` actually feeds the generators. `profile/Profile.tsx` renders `LessonNote` and `DefenseCard` (on results and the Freestyle end screen).
